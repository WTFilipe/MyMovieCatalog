...
class RemoveFromFavoriteUseCaseImplTest {
    ...
    fun setUp() {
        ...
        useCase = RemoveFromFavoriteUseCaseImpl(fakeRepository)
    ...
    @Test
    fun `when removing a movie from favorite, its favorite field should be false`() = runBlocking {
        val favoriteMovie = (fakeRepository as FakeMovieRepository).getPopularMoviesWithoutPaging().first().first { it.isFavorite }

        useCase.execute(favoriteMovie)

        val nowNotFavoriteMovie = (fakeRepository as FakeMovieRepository).getPopularMoviesWithoutPaging().first().first { it.id == favoriteMovie.id }

        assertEquals(false, nowNotFavoriteMovie.isFavorite)
    }

    @Test
    fun `when removing a invalid movie to favorites, should throw error`() = runBlocking {
